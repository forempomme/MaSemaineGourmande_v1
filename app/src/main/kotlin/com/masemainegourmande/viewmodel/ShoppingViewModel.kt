...
class ShoppingViewModel(private val repo: AppRepository) : ViewModel() {
    ...
    /** Called after drag ends — ONE coroutine writes ALL categories atomically, no race condition. */
    fun reorderAllCategories(orderedIds: List<String>) {
        viewModelScope.launch {
            val cats = repo.getCategories().sortedBy { it.sortOrder }
            val sorted = orderedIds.mapNotNull { id -> cats.find { it.id == id } }
            sorted.forEachIndexed { i, cat -> repo.upsertCategory(cat.copy(sortOrder = i)) }
        }
    }
