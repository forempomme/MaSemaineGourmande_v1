...
class ShoppingViewModel(private val repo: AppRepository) : ViewModel() {
    ...
    /** Set the complete item order for a category (called after drag ends). */
    fun setItemOrder(catId: String, orderedIds: List<String>) {
        _itemOrders[catId] = orderedIds
    }


    /**
     * Atomic reorder: sets sortOrder for ALL categories in one coroutine.
     * Called once after drag ends with the complete desired order.
     * Avoids the race condition of N separate reorderCategoryByIndex calls.
     */
    fun reorderAllCategories(orderedIds: List<String>) {
        viewModelScope.launch {
            val cats = repo.getCategories().associateBy { it.id }
            orderedIds.forEachIndexed { index, id ->
                cats[id]?.let { repo.upsertCategory(it.copy(sortOrder = index)) }
            }
        }
    }
